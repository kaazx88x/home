<ul class="nav nav-tabs">
    <li class="active"><a data-toggle="tab" href="#trans_ref">@lang('localize.transaction_reference')</a></li>
    @if($order->status == 1 || $order->status == 4)
        @if($type == 'admin')
        <li class="dropdown"><a class="nav-link dropdown-toggle" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false" href="#tax_inv">@lang('localize.tax_invoice')</a>
            <ul class="dropdown-menu">
                <li><a data-toggle="tab" href="#tax_inv">MM / EI</a></li>
                <li><a data-toggle="tab" href="#merchant_inv">@lang('localize.merchant_invoice')</a></li>
            </ul>
        </li>
        @else
        <li><a data-toggle="tab" href="#merchant_inv">@lang('localize.merchant_invoice')</a></li>
...
<div class="tab-content">
    <div id="trans_ref" class="tab-pane fade in active">
        ...
        @endif
            ...
            <div class="modal-footer">
                ...
                $add="n"
                ?>
                <a href="{{ url('download_pdf_inv/'.$order->id.'/'.$type.'/'.$type_inv.'/'.$add) }}" class="btn btn-primary">{{trans('localize.download')}}</a>
                <a href="{{ url('print_order_offline/'.$order->id.'/'.$type.'/'.$type_inv.'/'.$add) }}" class="btn btn-primary" onclick="window.open('{{ url('print_order_offline/'.$order->id.'/'.$type.'/'.$type_inv.'/'.$add) }}', 'newwindow', 'width=750, height=500'); return false;">{{trans('localize.print')}}</a>
                <button data-dismiss="modal" class="btn btn-default" type="button">{{trans('localize.close_')}}</button>
            </div>
        </div>
    </div>
    <div id="tax_inv" class="tab-pane fade">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
            <div class="row">
                <div class="col-sm-3"><img src="{{ url('/assets/images/meihome_logo.png') }}" alt="" style="width:auto; height:35px;"></div>
        ...
        <select id="company" name="company">
            ...
            <div class="row">
                <div class="col-sm-6" style="float:left;">
                    <b>@lang('localize.bill_to') : </b><div id="company_name" style="display:inline-block"></div><br/>
                    <b>@lang('localize.address') : </b><div id="company_add" style="display:inline-block"></div>
                    <b>@lang('localize.contact_number') : </b><div id="company_contact" style="display:inline-block"></div><br>
                    <b>@lang('localize.gst_no') : </b><div id="company_gst" style="display:inline-block"></div>
                </div>
                <div class="col-sm-6" style="border:1px solid black;margin-top:5%">
                    <b>@lang('localize.invoice_no') :</b> OFC{{ $order->tax_inv_no }}<br>
                    <b>@lang('localize.date')        :</b>{{ date_format($order->created_at, 'd/m/Y') }}<br>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <table border=1 width="100%">
                        ...
                        <tr>
                            <th colspan="3" style="padding-left:5px;">{{ trans('localize.offline_commission') }}</td>
        ...
        @endif
            ...
            <div class="modal-footer">
                ...
                $type_inv = "tax_inv";
                ?>
                <a href="{{ url('download_pdf_inv/'.$order->id.'/'.$type.'/'.$type_inv) }}" class="btn btn-primary" id="tax_pdf_link">{{trans('localize.download')}}</a>
                <a href="{{ url('print_order_offline/'.$order->id.'/'.$type.'/'.$type_inv) }}" class="btn btn-primary" onclick="window.open('{{ url('print_order_offline/'.$order->id.'/'.$type) }}', 'newwindow', 'width=750, height=500'); return false;" id="tax_print_link">{{trans('localize.print')}}</a>
                <button data-dismiss="modal" class="btn btn-default" type="button">{{trans('localize.close_')}}</button>
            </div>
        </div>
    </div>
    <div id="merchant_inv" class="tab-pane fade">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
            <div class="row">
                <div class="col-sm-3"><img src="{{ url('/assets/images/meihome_logo.png') }}" alt="" style="width:auto; height:35px;"></div>
...
$(function() {
    ...
    $.getJSON( "{{ url('/backend/js/company.json') }}", function( data ) {
        ...
        var link_pdf = "{{ url('download_pdf_inv/'.$order->id.'/'.$type.'/tax_inv/') }}/"+data[0].name;
        var link_print = "{{ url('print_order_offline/'.$order->id.'/'.$type.'/tax_inv/') }}/"+data[0].name;

        $('#company_name').text(data[0].company_name);
        $('#company_add').text(data[0].address);
        $('#company_contact').text(data[0].tel);
        $('#company_gst').text(data[0].gst_no);
        $('#tax_pdf_link').attr("href", link_pdf);
        $('#tax_print_link').attr("href", link_print);
        $('#tax_print_link').attr("onclick", "window.open('"+link_print+"', 'newwindow', 'width=750, height=500'); return false;");
        ...
        $.each(data , function (i, option){
            ...
            $('#company').append($('<option/>',
                {
                    "value" : option.name,
                    "data-company-name" :option.company_name,
                    "data-gst-no": option.gst_no,
                    "data-contact" : option.tel,
            ...
            $('#company').change(function () {
                link_pdf = "{{ url('download_pdf_inv/'.$order->id.'/'.$type.'/tax_inv/') }}/"+$('option:selected', this).data('value');
                link_print = "{{ url('print_order_offline/'.$order->id.'/'.$type.'/tax_inv/') }}/"+$('option:selected', this).data('value');
                $('#company_name').text($('option:selected', this).data('company-name'));
                $('#company_add').text($('option:selected', this).data('address'));
                $('#company_contact').text($('option:selected', this).data('contact'));
                $('#company_gst').text($('option:selected', this).data('gst-no'));
                $('#tax_pdf_link').attr("href", link_pdf);
                $('#tax_print_link').attr("href", link_print);
                $('#tax_print_link').attr("onclick", "window.open('"+link_print+"', 'newwindow', 'width=750, height=500'); return false;");
