...
<div class="wrapper wrapper-content animated fadeInRight">
    ...
    <div class="row">
        <div class="tabs-container">
            ...
            @include('merchant.product.nav-tabs', ['link' => 'description'])
                    ...
                    <div class="panel-body">
                        <div class="col-lg-12 nopadding">
                            <div class="col-sm-3 pull-right">
                                <button class="btn btn-block btn-primary" id="submit_top">@lang('localize.update')</button>
                            </div><br><br><br>
                        </div>

                        <form class="form" id="mer_edit_product" action="{{ url($route . '/product/description', [$product['details']->pro_id]) }}" method="POST" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="col-lg-12">
                                ...
                                <div class="ibox-content">
                                    <div class="tab-content">
                                        <div class="form-group tab-pane fade in active" id="en">
                                            <textarea id="desc_en" placeholder="@lang('localize.description_english')" class="form-control compulsary" id='pro_desc_en' name='pro_desc_en'>{!! old('pro_desc_en', $product['details']->pro_desc_en) !!}</textarea>
                                        </div>

                                        <div class="form-group tab-pane fade" id="cn">
                                            <label class="control-label">@lang('localize.description_chinese')</label>
                                            <textarea id="desc_cn" placeholder="@lang('localize.description_chinese')" class="form-control" name='pro_desc_cn'>{!! old('pro_desc_cn', $product['details']->pro_desc_cn) !!}</textarea>
                                        </div>

                                        <div class="form-group tab-pane fade" id="cnt">
                                            <label class="control-label">@lang('localize.description_chinese_traditional')</label>
                                            <textarea id="desc_cn" placeholder="@lang('localize.description_chinese_traditional')" class="form-control" name='pro_desc_cnt'>{!! old('pro_desc_cnt', $product['details']->pro_desc_cnt) !!}</textarea>
                                        </div>

                                        <div class="form-group tab-pane fade" id="my">
                                            <label class="control-label">@lang('localize.description_bahasa')</label>
                                            <textarea id="desc_my" placeholder="@lang('localize.description_bahasa')" class="form-control" name='pro_desc_my'>{!! old('pro_desc_my', $product['details']->pro_desc_my) !!}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-lg-5">
                            <div class="col-sm-6">
                                <button type="button" class="btn btn-block btn-default" onclick="reset()">@lang('localize.reset_form')</button>
                            </div>
                            <div class="col-sm-6">
                                <button type="button" class="btn btn-block btn-primary" id="submit">@lang('localize.update')</button>
                            </div>
                        </div>
        ...
        height: 1000,
            callbacks: {
                onImageUpload: function(files) {
                    var mer_id = parseInt("{{\Auth::guard('merchants')->user()->mer_id}}");
