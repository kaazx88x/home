        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    ...
                    <form id='form' class="form-horizontal" action='/admin/setting/banner/type/edit/{{$bannertype->id}}' method="POST">
                        {{ csrf_field() }}
                        <div class="form-group">
                            <label class="col-lg-2 control-label">{{trans('localize.Type_Name')}}</label>
                            <div class="col-lg-10">
                                <input type="text" placeholder="{{trans('localize.Type_Name')}}" class="form-control compulsary" name='name'  value='{{$bannertype->name}}'>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-lg-2 control-label">{{trans('localize.Description')}}</label>
                            <div class="col-lg-10">
                                <input type="text" placeholder="{{trans('localize.Description')}}" class="form-control" name="description" value='{{$bannertype->description}}'>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="col-lg-offset-2 col-lg-10">
                                @if($edit_permission)
                                <button class="btn btn-sm btn-primary form_submit" type="button">{{trans('localize.Edit')}}</button>
                                @endif
                            </div>
                        </div>
                    </form>
