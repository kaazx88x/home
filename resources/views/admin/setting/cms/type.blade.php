...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-sm-4">
        <h2>{{trans('localize.cms_management')}}</h2>
    </div>
</div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-stripped">
                            <thead>
                                <tr>
                                    <th>#ID</th>
                                    <th>{{trans('localize.cms_type')}}</th>
                                    <th>{{trans('localize.Description')}}</th>
                                    <th data-hide="phone" data-sort-ignore="true">{{trans('localize.Action')}}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($cms_types as $type)
                                <tr>
                                    <td>{{$type->id}}</td>
                                    <td>{{$type->name}}</td>
                                    <td>{{$type->description}}</td>
                                    <td>
                                        <!-- <a href="/admin/setting/cms/type/edit/{{$type->id}}" class="btn btn-white btn-sm"><i class="fa fa-pencil"></i> {{trans('localize.Edit')}}</a> -->
                                        <a href="/admin/setting/cms/manage/{{$type->id}}" class="btn btn-primary btn-sm"><i class="fa fa-list"></i> {{trans('localize.cms_manage')}} ({{$type->cms->count()}}) </a>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
